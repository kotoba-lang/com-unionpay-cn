(ns unionpay-cn.main
  "Kotodama WASM entrypoint for the Unionpay_cn clean-room actor (L4) — Clojure port.
  ...
  facts (`unionpay_cn.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "unionpay_cn")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"unionpay_cn.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "unionpay_cn-compat" :tier tier :entities entities} 200])
...
(defn start! [] :unionpay_cn-compat/ready)
