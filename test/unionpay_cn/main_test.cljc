(ns unionpay-cn.main-test
  "Contract + behavioral test for the unionpay-cn-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "unionpay_cn-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'unionpay-cn.main-test)]
